...
缺失字段一律按此序解决，**不要一发现「Fuyao 没有」就去别的接口补**：

```
① Fuyao 批量取  >  ② 批量算  >  ③ 其他接口批量  >  ④ Fuyao 逐个
```

详见 `DATA-SOURCING.md`。已纠正的具体误判：

| 我之前的结论 | 实际 |
|---|---|
| 「Fuyao 没有股本 → 必须保留东财」 | ❌ 跳过 ①②。`auction` 有 `float_market_cap`，**流通股本 = 市值/价**，56 请求即可 |
| 「换手率必须从东财补流通股本」 | ❌ 同上；且**股本只在该股送转/增发时变**，可从 `adjust_events` 派生 |
| 「市值要存起来」 | ❌ **用时实时算**（用户建议）：`股本(as of t) × 收盘(t)` |

**股本 = 慢变量（事件驱动），市值 = 快变量（每天）** —— 这个区分决定了
哪些落库、哪些实时算。
