...
## 六、实施结果（2026-09-17 实测）

按「Fuyao-only」方案实现并实跑，**未引入任何第三方接口**。

| 数据 | 命令 | 请求数 | 耗时 | 产物 |
|---|---|---|---|---|
| 证券列表 `symbol` | `xlp ingest symbol` | **1** | 1.1 s | 5,573 行 |
| 流通股本 `share_capital` | `xlp ingest share` | **56** | 12.1 s | 5,550 行 |
| 行业/概念/地域 `stock_tag` | `xlp ingest tags` | **516** | ~110 s | 81,489 条关系 |

### `symbol` 补齐（5,565 / 5,573 有行业）

```
industry       通用设备 / 汽车零部件 / 银行 / 白酒 …
area           浙江 / 江苏 / 北京 / 广东(除深圳) / 深圳 …
```
...
| tag | 指数数 | 关系数 |
|---|---|---|
| `cn_concept` | 390 | 70,370 |
| `region` | 33 | 11,108 |
| `industry`（仅 881xxx） | 90 | 5,565 |

每股标签数：3 个到 **69 个**（茅台：乡村振兴 / 国企改革 / 白酒概念 / 超级品牌 / 沪股通 …）。

### `share_capital` 精度实测

| 股票 | 反推流通股本 | 真实值 | 偏差 |
|---|---|---|---|
| `000001.SZ` 平安银行 | 19,405,684,991 | 19,405,918,198 | **-0.00%** |
| `600519.SH` 贵州茅台 | 1,250,081,601 | 1,256,197,800 | **-0.49%** |

偏差非系统性（取决于接口算市值所用价格与 `last_price` 的错位幅度）。
对换手率闸（8%）与市值闸（55亿/1000亿）够用，**不用于精确对账**。

> 23 只被跳过（市值或价格缺失，多为停牌）—— **缺失即缺失，不填 0**
> （否则换手率会被算成无穷大）。

### 请求数的诚实交代

`stock_tag` 的 516 次请求**无法再压缩**：

- Fuyao 没有「个股 → 所属指数」的反向接口
  （传股票代码给 `ths-index-constituents` 返回 **0 行**，实测）
- 成分股接口不接受逗号批量（`code=1002`）
- 已做两处削减：行业只取 `881xxx`（省 230 次）、`tszs` 不做（省 105 次）

**但这是一次性 + 每月任务**：110 秒换一张几 MB 的本地表，
之后每日板块/概念涨跌用本地 K 线算，**零请求**。

### 顺带验证：压实对新表同样有效

重复摄入会产生多版本（`stock_tag` 2 → 1、`symbol` 2 → 1）。
`xlp compact run` 一次全部收拢，读取自动切快路径。
